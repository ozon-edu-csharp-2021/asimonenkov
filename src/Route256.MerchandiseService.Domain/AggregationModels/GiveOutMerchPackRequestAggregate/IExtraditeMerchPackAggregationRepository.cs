...
namespace Route256.MerchandiseService.Domain.AggregationModels.GiveOutMerchPackRequestAggregate
{
    ...
    public interface IExtraditeMerchPackAggregationRepository : IRepository<ExtraditeMerchPackRequest>
    {
        ...
        /// <param name="cancellationToken">Токен для отмены операции. <see cref="CancellationToken"/></param>
        /// <returns>Объект запроса</returns>
        Task<IReadOnlyList<ExtraditeMerchPackRequest>> GetByIdAsync(RequestNumber number, CancellationToken cancellationToken = default);
        ...
        /// <param name="cancellationToken">Токен для отмены операции. <see cref="CancellationToken"/></param>
        /// <returns>Объект запроса</returns>
        Task<IReadOnlyList<ExtraditeMerchPackRequest>> GetByEmployeeIdAndMerchPackNameAsync(EmployeeId employeeId, MerchPackName merchPackName, CancellationToken cancellationToken = default);
    }
}